...
export class LAppModel extends CubismUserModel {
  ...
  public hitTest(hitArenaName: string, x: number, y: number): boolean {
    ...
    for (let i = 0; i < count; i++) {
      // console.log(this._modelSetting.getHitAreaName(i));
      // console.log(count); //count为2才正常
      if (this._modelSetting.getHitAreaName(i) == hitArenaName) {
        const drawId: CubismIdHandle = this._modelSetting.getHitAreaId(i); //只遍历一次，异常，正常情况下是两次
        console.log(drawId); //问题所在
        // console.log(this.isHit(drawId, x, y));
